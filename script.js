...
async function onFeedSearchInput(event) {
  ...
  searchTimer = setTimeout(async () => {
    ...
    try {
      /* Hashtag search */
      if (q.startsWith('#')) {
        const tag = q.slice(1).replace(/^#+/, '').trim();
        if (!tag) {
          if (resultsEl) {
            resultsEl.innerHTML = '';
            resultsEl.style.display = 'none';
        ...
        if (resultsEl) {
          resultsEl.innerHTML = found.length
            ? `<div class="search-result-empty">#${escapeHtml(tag)} — ${found.length}টি post পাওয়া গেছে</div>`
            : `<div class="search-result-empty">#${escapeHtml(tag)} — কোনো post পাওয়া যায়নি।</div>`;
        }
      ...
      /* Username search.
         The app has a legacy username-keyed collection under
         artifacts/{appId}/public/data/users. Searching document IDs avoids
         requiring an orderBy(username) index and also avoids a permission
         failure on the canonical UID-keyed collection in stricter rules. */
      ...
      /* Exact canonical fallback. This is only used when the legacy lookup
         returned nothing, so a stricter ruleset is less likely to block
      ...
      if (!users.length) {
        try {
          const canonicalSnap = await db.collection('users')
            ...
            .limit(12)
            .get();
          users = canonicalSnap.docs.map(d => ({
            id: d.id,
            ...d.data(),
            _source: 'canonical'
          }));
        } catch (canonicalError) {
          console.warn('CANONICAL_SEARCH_SKIPPED', canonicalError);
        }
      }
      ...
      if (resultsEl) {
        if (!users.length) {
          ...
          /* Event delegation keeps usernames with special characters safe. */
      ...
      /* Do not replace the feed with a single result.
         Keep the normal feed visible while the dropdown shows user matches. */
      feedFilter = '';
      renderFeed();
