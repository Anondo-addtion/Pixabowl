/* =========================================================
   PIXABOWL APP / WEBSITE MODE
   ---------------------------------------------------------
   Website URL:
     https://anondo-addition.github.io/

   Android App/WebView URL:
     https://anondo-addition.github.io/?mode=app

   No original app logic is changed here. The query parameter
   decides which layout CSS is active.
   ========================================================= */
(function setupPixabowlMode() {
  const params = new URLSearchParams(window.location.search);
  const isAppMode = params.get('mode') === 'app';

  // Add the class to <html> immediately so the browser layout
  // can be applied before the rest of the page initializes.
  document.documentElement.classList.toggle('web-mode', !isAppMode);
  document.documentElement.dataset.pixabowlMode = isAppMode ? 'app' : 'web';

  function syncBodyMode() {
    if (!document.body) return;
    document.body.classList.toggle('web-mode', !isAppMode);
    document.body.dataset.pixabowlMode = isAppMode ? 'app' : 'web';
  }

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', syncBodyMode, { once: true });
  } else {
    syncBodyMode();
  }

  console.log(`Pixabowl mode: ${isAppMode ? 'APP' : 'WEB'}`);

  if (!isAppMode) {
    const ensureWebNavigation = () => {
    ...
    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', ensureWebNavigation, { once: true });
    } else {
      ensureWebNavigation();
    }
  }
...
function openGuidelinesModal(event) {
...
function updateSignupButtonState() {
  const checkbox = document.getElementById('signup-rules-checkbox');
  const button = document.getElementById('btn-create-account');
  if (!button) return;
...
function markCommunityRulesAccepted() {
  try {
    localStorage.setItem('pixabowl_rules_version', COMMUNITY_RULES_VERSION);
    localStorage.setItem('pixabowl_rules_accepted_at', new Date().toISOString());
  } catch (e) {}
}
...
async function handleSignUp() {
  ...
  const rulesCheckbox = document.getElementById('signup-rules-checkbox');
  if (!rulesCheckbox?.checked) return showAlert('Account তৈরি করতে Community Guidelines মেনে নেওয়া প্রয়োজন।');
  ...
  try {
    ...
    // Store the accepted rules version in the user's own profile.
    await db.collection('users').doc(credential.user.uid).set({
      communityRulesVersion: COMMUNITY_RULES_VERSION,
      communityRulesAcceptedAt: firebase.firestore.FieldValue.serverTimestamp()
    }, {merge: true});
    markCommunityRulesAccepted();
  ...
  container.innerHTML = posts.map(post => {
    ...
    return `<article class="post-card" data-post-id="${escapeHtml(post.id)}">
      ...
      <div class="post-actions">
        ...
        <button type="button" class="post-id-tag comment-toggle-btn" onclick="toggleComments('${escapeHtml(post.id)}')">💬 ${comments.length} comment${comments.length === 1 ? '' : 's'}</button>
      </div>
      <div class="comments-panel" id="comments-${escapeHtml(post.id)}" style="display:${openCommentPosts.has(post.id) ? 'block' : 'none'};">
        <div class="comments-list">
          ${comments.length ? comments.slice(-20).map(comment => `
            <div class="comment-item"><span class="user">${escapeHtml(comment.username || 'user')}</span>${escapeHtml(comment.text || '')}</div>
          `).join('') : '<div class="comments-empty">এখনও কোনো comment নেই। প্রথম comment করুন!</div>'}
        </div>
        <div class="add-comment-box">
          <div class="comment-input-wrap">
            <input type="text" id="comment-input-${escapeHtml(post.id)}" maxlength="500" placeholder="Write a comment..." autocomplete="off" onkeydown="handleCommentKeydown(event, '${escapeHtml(post.id)}')">
          </div>
          <button type="button" class="comment-send-btn" onclick="submitComment('${escapeHtml(post.id)}')">Send</button>
        </div>
      </div>
    </article>`;
  }).join('');
...
function toggleComments(postId) {
  if (!currentUser) return showAlert('Comment করতে আগে Log in করুন।');
  if (!postId) return;
  if (openCommentPosts.has(postId)) openCommentPosts.delete(postId);
  else openCommentPosts.add(postId);
  renderFeed();
...
async function submitComment(postId) {
  ...
  try {
    ...
    await db.runTransaction(async tx => {
      ...
      comments.push({
        uid: currentUser.uid,
        username: me.username || currentUser.email?.split('@')[0] || 'user',
        text,
        createdAt: new Date().toISOString()
      });
      // Keep the post document small while retaining recent comments.
      tx.update(ref, {comments: comments.slice(-100)});
    });
    ...
    showAlert(`Comment করা যায়নি: ${getErrorMessage(e)}`);
  }
}
...
async function onFeedSearchInput(event) {
  ...
  searchTimer = setTimeout(async () => {
    ...
    if (resultsEl) {
      ...
      resultsEl.innerHTML = '<div class="search-result-empty">Search হচ্ছে... 🔎</div>';
    }

    try {
      /*
       * SMART HASHTAG SEARCH
       *
       * Old behavior was exact-match only:
       *   #travel -> array-contains "travel"
       *
       * Now it is prefix/case-insensitive:
       *   #a  -> #art, #anime, #animals, ...
       *   #ad -> #admin, #adventure, ...
       *
       * Firestore cannot do a startsWith query inside an array field, so
       * hashtags are filtered client-side after reading the current posts.
       * If nothing matches, the normal "no post found" behavior remains.
       */
      if (q.startsWith('#')) {
        const tagQuery = q.slice(1).replace(/^#+/, '').trim();
        if (!tagQuery) {
          if (resultsEl) {
            resultsEl.innerHTML = '';
            resultsEl.style.display = 'none';
        ...
        const snap = await POSTS_COLL.get();
        const found = snap.docs
          .map(d => ({id: d.id, ...d.data()}))
          .filter(post => Array.isArray(post.hashtags) && post.hashtags.some(h =>
            String(h).replace(/^#/, '').trim().toLowerCase().startsWith(tagQuery)
          ))
          .sort((a,b) => createdAtMillis(b.createdAt) - createdAtMillis(a.createdAt));

        if (resultsEl) {
          resultsEl.innerHTML = found.length
            ? `<div class="search-result-empty">#${escapeHtml(tagQuery)} — ${found.length}টি matching post পাওয়া গেছে</div>`
            : `<div class="search-result-empty">#${escapeHtml(tagQuery)} — কোনো post পাওয়া যায়নি।</div>`;
        }
      ...
      /*
       * SMART USERNAME / ID SEARCH
       *
       * Prefix matching means:
       *   a  -> every username starting with "a"
       *   ad -> every username starting with "ad"
       *   @a -> same as "a"
       *
       * No result limit is used here, so all matching legacy username-keyed
       * documents can appear. This keeps the old Firestore structure intact.
       */
      const usernameQuery = q.replace(/^@/, '').trim();
      if (!usernameQuery) return;

      const end = usernameQuery + '\uf8ff';
      const legacySnap = await USERS_COLL
        .orderBy(firebase.firestore.FieldPath.documentId())
        .startAt(usernameQuery)
        .endAt(end)
        .get();

      let users = legacySnap.docs.map(d => ({
        id: d.id,
        ...d.data(),
        _source: 'legacy'
      }));
      ...
      /*
       * Canonical UID-keyed collection fallback.
       * Unlike the old exact-match fallback, this also does prefix matching
       * on username, so "a" and then "ad" both return all matching IDs.
       */
      try {
        const canonicalSnap = await db.collection('users')
          .orderBy('username')
          .startAt(usernameQuery)
          .endAt(end)
          .get();

        const canonicalUsers = canonicalSnap.docs.map(d => ({
          id: d.id,
          ...d.data(),
          _source: 'canonical'
        }));

        /* Merge both stores without showing the same username twice. */
        const seen = new Set(users.map(u => normalizeUsername(u.username || u.id).toLowerCase()));
        canonicalUsers.forEach(user => {
          const key = normalizeUsername(user.username || user.id).toLowerCase();
          if (!seen.has(key)) {
            users.push(user);
            seen.add(key);
          }
        ...
        console.warn('CANONICAL_PREFIX_SEARCH_SKIPPED', canonicalError);
      }

      if (resultsEl) {
        if (!users.length) {
          /* Keep the existing no-result behavior/text. */
          resultsEl.innerHTML = '<div class="search-result-empty">কোনো username পাওয়া যায়নি।</div>';
        } else {
          resultsEl.innerHTML = users.map((user, index) => {
            const username = normalizeUsername(user.username || user.id);
            const fullName = user.fullName || '';
            const avatar = user.avatarUrl || '';
            return `
              <div class="search-result-item" data-search-index="${index}">
                <div class="search-result-avatar" ${avatar ? `style="background-image:url('${escapeHtml(avatar)}')"` : ''}>
                  ${avatar ? '' : '👤'}
                </div>
                <div>
                  <div class="search-result-name">@${escapeHtml(username)}</div>
                  ${fullName ? `<div class="search-result-sub">${escapeHtml(fullName)}</div>` : ''}
                </div>
              </div>`;
          }).join('');

          resultsEl.querySelectorAll('.search-result-item').forEach((item, index) => {
            item.addEventListener('click', () => selectSearchUser(users[index].username || users[index].id));
          });
        }
      }

      /* Keep the normal feed visible while search results appear in the dropdown. */
      feedFilter = '';
      renderFeed();
      ...
      if (resultsEl) {
        resultsEl.innerHTML = `<div class="search-result-empty">Search করা যায়নি।<br>${escapeHtml(getErrorMessage(e))}</div>`;
      }
    }
  }, 220);
}
async function selectSearchUser(username) {
...
Object.assign(window, {
  ...
  openGuidelinesModal, closeGuidelinesModal, updateSignupButtonState,
  toggleComments, handleCommentKeydown, submitComment, selectSearchUser
});
...
/* Android/WebView pull-to-refresh protection.
   Important: if the native wrapper (HopWeb) itself has a SwipeRefresh/
   Pull-to-refresh option enabled, the wrapper can refresh before the page
   receives the gesture. This JS/CSS guard protects the web page side. */
(function installPullRefreshGuard() {
  let startY = 0;
  let blockingPull = false;

  document.addEventListener('touchstart', (e) => {
    if (e.touches.length !== 1) return;
    startY = e.touches[0].clientY;

    // App mode keeps the original .screen scroll container.
    // Web mode uses the browser document, so never block normal page scrolling.
    if (document.documentElement.dataset.pixabowlMode !== 'app') {
      blockingPull = false;
      return;
    }

    const scrollHost =
      e.target?.closest?.('.screen.active') ||
      document.querySelector('.screen.active');

    blockingPull = !!scrollHost && scrollHost.scrollTop <= 1;
  }, {passive: true});

  document.addEventListener('touchmove', (e) => {
    if (!blockingPull || e.touches.length !== 1) return;
    ...
    if (scrollHost.scrollTop <= 1 && dy > 0) {
      e.preventDefault();
    } else if (dy <= 0 || scrollHost.scrollTop > 1) {
      blockingPull = false;
    }
  }, {passive: false});
