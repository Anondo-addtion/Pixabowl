...
async function handleForgotPassword(event) {
  ...
  if (!login) {
    showAlert('Password reset করতে আপনার Email অথবা Username লিখুন।');
    loginEl?.focus();
    return false;
  }

  showLoading(true, 'Password reset প্রস্তুত হচ্ছে...');
  try {
    let email = login.toLowerCase();

    // Email is the most reliable reset identifier. If a username is supplied,
    // resolve it through the canonical UID profile first, then the legacy profile.
    if (!login.includes('@')) {
      const profile = await findUserByUsername(login);
      if (!profile) {
        throw new Error('এই username-এর profile পাওয়া যায়নি। Password reset করতে account-এর email address লিখুন।');
      }
      if (!profile.email) {
        throw new Error('এই profile-এ email address সংরক্ষিত নেই। Password reset করতে account-এর email address লিখুন।');
      }
      email = String(profile.email).trim().toLowerCase();
    }

    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      throw new Error('সঠিক email address দিন।');
    }
    ...
    showAlert(
      `Password reset link ${email} ঠিকানায় পাঠানো হয়েছে।\n\n` +
      'Inbox-এর পাশাপাশি Spam/Junk folder-ও দেখুন। Email পেতে কয়েক মিনিট সময় লাগতে পারে।'
    );
    return true;
  } catch (e) {
    ...
    if (code === 'auth/user-not-found') {
      message = 'এই email দিয়ে কোনো account পাওয়া যায়নি। Email address ঠিক আছে কি না দেখুন।';
    } else if (code === 'auth/operation-not-allowed') {
      message = 'Firebase-এ Email/Password Authentication চালু করা নেই। Firebase Console → Authentication → Sign-in method থেকে Email/Password Enable করুন।';
    } else if (code === 'auth/invalid-email') {
      message = 'Email address-টি সঠিক নয়।';
    } else if (code === 'auth/too-many-requests') {
      message = 'অনেকবার reset request হয়েছে। কিছুক্ষণ অপেক্ষা করে আবার চেষ্টা করুন।';
    } else if (code === 'auth/network-request-failed') {
      message = 'Internet connection সমস্যা। Connection ঠিক করে আবার চেষ্টা করুন।';
    } else if (code === 'auth/unauthorized-continue-uri') {
      message = 'Firebase-এর Authorized domains-এ এই website domain যোগ করা নেই। Firebase Console → Authentication → Settings → Authorized domains-এ domain যোগ করুন।';
    }

    showAlert(`Password reset করা যায়নি:\n\n${message}`);
    return false;
  } finally {
    showLoading(false);
  }
